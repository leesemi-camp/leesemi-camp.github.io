...
이 문서는 다음 3가지를 “조사 시작” 관점에서 정리합니다.

1) `data/` 폴더의 파일이 **언제/어디에서** 사용되는지  
2) 이 저장소에서 Hotspot(현안 지점) 데이터가 **어떻게 관리되는지**  
3) Hotspot(점) 외에 Route/Trajectory(선/궤적) 데이터를 지원하려면 **무엇을 준비해야 하는지**
...
## 2) Hotspot(현안 지점) 데이터는 어떻게 관리되는가

### 2.1 데이터 저장소/권한 모델

- 저장소: Firestore 컬렉션 기본값 `crowd_hotspots`
...
- 필수(사실상) 좌표/식별
  ...
  - 문서 id는 Firestore 자동 생성(추가) 또는 기존 doc id(수정)
- 내용
  - `title`(string)
  - `memo`(string)
  ...
  - `issueRefId`(string, 외부 카탈로그 연동 id)
- 동/행정동 메타
  - `dongName`(string)
  - `emdCode`(string)
  - `dongSelectionMode`: `"auto" | "manual" | "common"`
  - `dongKey`: `"__common__"` 또는 수동 지정 시 key
- 감사/최종 갱신
  - `updatedBy`(email)
  - `updatedAt`(serverTimestamp)

주의: `processHotspotSnapshot()`은 호환을 위해 `snake_case`(예: `dong_name`, `emd_cd`, `category_id`)도 함께 읽습니다.
...
- 렌더:
  ...
  - 리스트: `app.js#L4159 renderHotspotList(hotspots)`
  - 그룹 보기: `app.js#L4222 renderIssueGroupList(hotspots)` + `buildIssueGroups`
- 캐시/상태:
...
- 추가/수정
  - `handleHotspotSubmit()`에서 `update(payload)` 또는 `add(payload)`
...
## 3) Route/Trajectory(경로/궤적) 지원을 위해 필요한 준비
