...
- `http://localhost:5173/` (현안 열람)
- `http://localhost:5173/edit/` (현안 수정)
...
- `/edit/`는 Firebase Auth 로그인 후 `staff=true` 커스텀 클레임(Custom Claim)이 있는 사용자만 접근 가능합니다.
...
- `index.html`: 공개 현안도(View)
- `edit/`: 현안 수정 지도(Edit)
- `map/`: 과거 현안도 주소 redirect
- `app.js`: 핵심 로직
- `config.js`: 환경 설정
